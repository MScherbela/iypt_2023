#%%
import numpy as np
import jax
import jax.numpy as jnp
import chex
import matplotlib.pyplot as plt


@chex.dataclass
class ModelParams:
    I_phi: float
    I_theta: float
    M: float
    s1: float
    s2: float
    sc: float
    r1: float
    r2: float
    alpha: float
    mu: float
    mu_roll: float
    g: float = 981.0


def get_rot_matrix(phi):
    c, s = jnp.cos(phi), jnp.sin(phi)
    return jnp.array([[c, -s], [s, c]])


def get_acceleration(R, phi, theta, R_dot, phi_dot, theta_dot, p: ModelParams):
    U = get_rot_matrix(phi)

    R_dot_moving_frame = U.T @ R_dot
    radii = jnp.array([p.r1, p.r2])
    distances = jnp.array([p.s1, p.s2])

    v_rolling = radii * theta_dot
    v_slip_parallel = R_dot_moving_frame[0]  + (distances - p.sc) * phi_dot - v_rolling
    v_slip_normal = R_dot_moving_frame[1]

    F_normal = p.M * p.g * jnp.cos(p.alpha) * (distances[::-1] - p.sc) / (distances[::-1] - distances)

    #sign_func = lambda x: jnp.tanh(x / 0.1)
    sign_func = lambda x: jnp.sign(x)
    F_fric_parallel = -p.mu * F_normal * sign_func(v_slip_parallel)
    # F_fric_parallel -= p.mu_roll * F_normal * sign_func(v_rolling)
    F_fric_normal = -p.mu * F_normal * sign_func(v_slip_normal)

    theta_acc = -jnp.dot(F_fric_parallel, radii) / p.I_theta
    # theta_acc -= p.mu_roll * jnp.sign(theta_dot)
    theta_acc -= p.mu_roll * theta_dot
    phi_acc = jnp.dot(F_fric_parallel, distances - p.sc) / p.I_phi

    F_fric_total_in_moving_frame = jnp.array([jnp.sum(F_fric_parallel), jnp.sum(F_fric_normal)])
    R_acc = p.g * jnp.sin(p.alpha) * np.array([0, -1]) + U @ F_fric_total_in_moving_frame / p.M
    return R_acc, phi_acc, theta_acc


@jax.jit
def time_step(R, phi, theta, R_dot, phi_dot, theta_dot, dt, p: ModelParams):
    R_acc, phi_acc, theta_acc = get_acceleration(R, phi, theta, R_dot, phi_dot, theta_dot, p)

    R_dot += R_acc * dt
    phi_dot += phi_acc * dt
    theta_dot += theta_acc * dt

    R += R_dot * dt
    phi += phi_dot * dt
    theta += theta_dot * dt
    return R, phi, theta, R_dot, phi_dot, theta_dot


m1 = 10
m2 = 30
s1 = 0
s2 = 5
M = m1 + m2
sc = 1 / M * (m1 * s1 + m2 * s2)
r1 = 0.2
r2 = 3
I_theta = 1 / 2 * m1 * r1**2 + 1 / 2 * m2 * r2**2
I_phi = m1 * (s1 - sc) ** 2 + m2 * (s2 - sc) ** 2

params = ModelParams(
    M=M, 
    I_phi=I_phi, 
    I_theta=I_theta, 
    s1=s1, 
    s2=s2, 
    sc=sc, 
    r1=r1, 
    r2=r2, 
    mu=0.4,
    mu_roll=1.0,
    alpha=np.radians(25)
)
dt = 1e-3
t_end = 2.0
t_values = np.arange(0, t_end, dt)
n_t = len(t_values)
R_values = np.zeros((n_t, 2))
phi_values = np.zeros(n_t)
theta_values = np.zeros(n_t)
R_dot_values = np.zeros((n_t, 2))
phi_dot_values = np.zeros(n_t)
theta_dot_values = np.zeros(n_t)

R_values[0, :] = np.array([0.0, 0.0])
phi_values[0] = np.radians(120)
R_dot_values[0, :] = [0.0, 0.0]

for i in range(n_t - 1):
    (
        R_values[i + 1, :],
        phi_values[i + 1],
        theta_values[i + 1],
        R_dot_values[i + 1, :],
        phi_dot_values[i + 1],
        theta_dot_values[i + 1],
    ) = time_step(
        R_values[i, :],
        phi_values[i],
        theta_values[i],
        R_dot_values[i, :],
        phi_dot_values[i],
        theta_dot_values[i],
        dt,
        params,
    )

x1 = R_values + np.stack([np.sin(phi_values), -np.cos(phi_values)], axis=1) * (params.s1 - params.sc)
x2 = R_values + np.stack([np.sin(phi_values), -np.cos(phi_values)], axis=1) * (params.s2 - params.sc)


plt.close("all")
fig, axes = plt.subplots(2, 3, figsize=(14, 10))
axes[0, 0].plot(t_values, R_values[:, 0])
axes[0, 1].plot(t_values, R_values[:, 1])
axes[0, 2].plot(t_values, phi_values / (2 * np.pi))
axes[1, 0].plot(t_values, theta_values / (2 * np.pi))
axes[1, 1].plot(R_values[:, 0], R_values[:, 1])


# axes[1, 1].plot(R_values[:, 0], R_values[:, 1])


for ax, label in zip(axes.flatten(), ["x", "y", "phi", "theta"]):
    ax.ticklabel_format(useOffset=False)
    ax.set_xlabel("t / sec")
    ax.set_ylabel(label)

fig, ax = plt.subplots(figsize=(17, 5))
n_skip = 20
fontsize=16

ax.plot(x1[0:i:n_skip, 1], x1[0:i:n_skip, 0], color='C0', alpha=0.3)    
ax.plot(x2[0:i:n_skip, 1], x2[0:i:n_skip, 0], color='C1', alpha=0.3)
marker1, = plt.plot([], [], marker='o', markersize=int(20*m1/M), color='C0')
marker2, = plt.plot([], [], marker='o', markersize=int(20*m2/M), color='C1')
connection, = plt.plot([], [], color='k')
text = ax.text(0.02, 0.95, "", transform=ax.transAxes, fontsize=fontsize)
ax.axis("equal")
ax.invert_xaxis()
ax.set_xlabel("Position along the ramp / cm", fontsize=fontsize)
ax.set_ylabel("Position across the ramp / cm", fontsize=fontsize)
ax.tick_params(labelsize=fontsize)

for i in range(0, n_t, n_skip):
    marker1.set_data([x1[i, 1]], [x1[i,0]])
    marker2.set_data([x2[i, 1]], [x2[i,0]])
    connection.set_data([x1[i, 1], x2[i, 1]], [x1[i,0], x2[i,0]])
    text.set_text(f"t = {t_values[i]:3.2f} sec")
    plt.pause(0.002)



# %%
